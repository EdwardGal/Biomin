...
//More-Less========================================================================================================================================================
const itemsList = document.querySelector('.clinical__lines');
if (itemsList) {
	const itemsListShow = itemsList.dataset.show;
	const itemsMore = document.querySelector('.clinical__button');
	...
	function setHeight(update = false) {
		const itemsListItems = document.querySelectorAll('.clinical__line');
...
//========================================================================================================================================================
//ДОБАВЛЕНИЕ ТОВАРОВ+ПЕРЕСЧЕТ ЦЕНЫ========================================================================================================================================================
const productsBtn = document.querySelectorAll('.product__button_buy');
...
let price = 0;
//========================================================================================================================================================
let checkBox = document.querySelector('.checkbox__input[value="1"]');
let totalPrice = document.querySelector('.form__price-cost-total');
checkBox.addEventListener("click", function (e) {
	checkBox = Number(600);
	const deliveryCost = document.querySelector('.form__price-cost-delivery');
	deliveryCost.textContent = checkBox;
});
...
const printFullPrice = () => {
	...
	cartPrice.textContent = `${normalPrice(price)} ₽`;
};
...
const deleteProducts = (productParent) => {
	...
	document.querySelector(`.product[data-id="${id}"]`).querySelector('.product__button_buy').disabled = false;
...
productsBtn.forEach(el => {
	...
	el.addEventListener('click', (e) => {
		...
		let img = parent.querySelector('.product__item-select-one').getAttribute('src');
		let title = parent.querySelector('.product__title').textContent;
		let priceNumber = parseInt(priceWithoutSpaces(parent.querySelector('.product__price-current').textContent));
...
let mainScreen = new Swiper('.main-screen__slider', {
	...
	navigation: {
		nextEl: '.slider-navigation__arrow-right',
		prevEl: '.slider-navigation__arrow-left',
	},
...
let popupMore = new Swiper('.more-popup__slider', {
	effect: 'fade',
	thumbs: {
		swiper: {
			el: '.more-popup__mini-slider',
			slidesPerView: 4,
		}
	},
	observer: true,
	...
	navigation: {
		nextEl: '.more-popup__arrow-right',
		prevEl: '.more-popup__arrow-left',
	},
});
...
async function form_submit(e) {
	...
	if (error == 0) {
		...
		if (ajax) {
			...
			let response = await fetch('sendmail.php',formAction, {
				method: formMethod,
				body: formData
			});
			if (response.ok) {
		...
		if (form.querySelector('.form__alarm')) form.querySelector('.form__alarm').remove();
		...
		if (form.querySelector('.form__alarm')) form.querySelector('.form__alarm').remove();
		form.querySelector('.form__button').insertAdjacentHTML('beforebegin', `<div class="form__alarm">Пожалуйста, заполните все обязательные поля</div>`);
		e.preventDefault();
	}
}
	...
	if (menuArrows.length > 0) {
		for (let index = 0; index < menuArrows.length; index++) {
			...
			document.addEventListener("click", function(e) {
				...
				if( event.target.classList != 'menu__arrow'){
...
testWebP(function (support) {
	if (support === true) {
		...
		document.querySelector('html').classList.remove('_no-webp');
	}
});
...
//// Прокрутка при клике
const menuLinks = document.querySelectorAll('.menu__link,.menu__sublink,.main-screen__item,.article-popup__item[data-goto]');
